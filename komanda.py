...
def calculate_tab(table: list[dict[int, int]]) -> float or int:
    order_value = 0

    for item in menu:
        for consumed_item in table:
            if(consumed_item["id"] == item["id"]):
                order_value += item["price"] * consumed_item["amount"]
